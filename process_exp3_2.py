...
def initialize_parameters():
    ...
    keyframe_interval = 1 # choice examples: [2, 3, 5, 8]
    enlarge_scale = 0.2
...
def light_track(pose_estimator,
    ...
    while img_id < num_imgs-1:
        ...
        if flag_keep_tracking:
        ...
        elif is_keyframe(img_id, 1) or flag_mandatory_keyframe:
            flag_mandatory_keyframe = False
            ...
            tmp_bbox_dets_list = copy.deepcopy(bbox_dets_list_list[img-1])
            ...
            for det_id in range(num_dets):
                ...
                keypoints = precomputed_dets[img_id][det_id]["keypoints"]
                keypoints_gt = keypoints

                if img_id == 0:
                    track_id = next_id
                    tracking_id_list.append(track_id)
                    next_id += 1
                else:
                    track_id, tmp_bbox_dets_list = get_track_id_SpatialConsistency_3d(bbox_3d_det, tmp_bbox_dets_list, img_id)
                    if track_id == -1:
                        track_id = get_track_id_SpatialConsistency(bbox_gt, tmp_bbox_dets_list, img_id)
                    # if track_id == -1:
                    #     track_id = get_track_id_SGCN(bbox_gt, bbox_dets_list_list, keypoints_gt, keypoints_list_list, img_id)

                    if track_id != -1:
                        if track_id not in tracking_id_list:
                            tracking_id_list.append(track_id)
                        else:
                            # duplicate ID in the same frame
                            track_id = next_id
                            next_id += 1         
    ...
    if flag_visualize is True:
        ...
        show_all_withIdDepth_from_standard_json(output_json_path, classes, joint_pairs, joint_names, image_folder, visualize_folder, flag_track = True)
...
def get_track_id_SpatialConsistency_3d(bbox_3d_gt, bbox_dets_list, img_id):
    # get bboxes from previous frame
    bbox_dets_list = bbox_dets_list
    ...
    if max_iou_score > thresh:
        match_bbox_det = bbox_dets_list_list[img_id - 1].pop(max_index)
        return match_bbox_det["track_id"], bbox_dets_list_list
    else:
        return -1, bbox_dets_list_list
...
def pose_to_standard_mot(keypoints_list_list, dets_list_list):
    ...
    for i in range(num_dets_list):
        ...
        for j in range(num_dets):
            ...
            candidate["bbox_3d"] = dets_dict["bbox_3d"]
...
if __name__ == '__main__':
    ...
    if args.dataset_split == "posetrack18_val":
        ...
        output_json_folder = "data/Data_2018/posetrack_results/lighttrack/results_exp3_2_openSVAI"
       
        print('input: ', detections_CPNformat_folder)
        print('output: ', output_json_folder)
    ...
    visualize_folder = "data/Data_2018/posetrack_results/lighttrack/visualize_exp3_2/"
    output_video_folder = "data/Data_2018/videos_exp3_2/"
    if not osp.exists(output_video_folder) and flag_visualize:
